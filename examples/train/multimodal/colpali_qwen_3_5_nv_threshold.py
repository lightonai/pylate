"""ColPali-style (multi-vector) training for Qwen3.5-4B via PyLate, with the
nv_threshold hard-negative retriever ported from
``examples/train/supervised_from_dense.py`` (``KDToContrastive``).

Same backbone, fix, loss and eval as ``colpali_qwen_3_5.py``. The only change is
the data side: instead of blindly taking ``document_ids[1]`` as the negative,
each query's negatives are selected from the KD scores with the nv_threshold
rule:

    a candidate document is a valid negative IFF
        score < NV_THRESHOLD * positive_score

where ``positive_score = scores[0]`` (the positive is always
``document_ids[0]``). This drops candidates whose teacher score is too close to
the positive — i.e. likely false negatives — exactly as the text retriever does
via ``KDToContrastive.map_to_query_positive_negatives``. We additionally require
at least ``NUM_NEGATIVES`` valid negatives per query (the
``has_enough_negatives`` filter) and emit them as ``negative_0 ..
negative_{NUM_NEGATIVES-1}`` columns.
...
Usage:
    accelerate launch --mixed_precision=bf16 scripts/train/colpali_qwen_3_5_nv_threshold.py
"""
...
RUN_NAME = f"qwen3_5_4b_colpali_contrastive_nv_threshold_{CHAT_TEMPLATE_VARIANT}"
...
# ── nv_threshold retriever config ────────────────────────────────────────────
# Ported from KDToContrastive in examples/train/supervised_from_dense.py.
# A candidate doc is kept as a negative only if its teacher score is below
# NV_THRESHOLD * positive_score (filters out likely false negatives). Each query
# must yield at least NUM_NEGATIVES such negatives or it is dropped.
# NOTE: every negative is an image, so each one adds a full ViT+LM forward under
# GradCache — keep NUM_NEGATIVES small for the 4B backbone.
NV_THRESHOLD = 0.95
NUM_NEGATIVES = 1
...
def load_split(
    ...
    """Load one KD split with nv_threshold hard-negative selection.

    Positive is ``document_ids[0]`` (score ``scores[0]``). Negatives are the
    first ``NUM_NEGATIVES`` candidates whose teacher score is strictly below
    ``NV_THRESHOLD * positive_score`` — the rule from ``KDToContrastive``. Rows
    that cannot supply that many valid negatives are dropped
    (``has_enough_negatives``). Images are decoded lazily via ``set_transform``.
    """
    ...
    for i in range(len(scores_ds)):
        ...
        # Positive is always the first document; threshold is relative to it.
        positive_score = scores[0]
        pos_filename = all_doc_filenames[doc_index[doc_ids[0]]]
        if pos_filename not in image_index:
            continue

        # nv_threshold: keep candidates whose score < NV_THRESHOLD * positive.
        neg_idxs: list[int] = []
        for j in range(1, len(doc_ids)):
            if scores[j] < NV_THRESHOLD * positive_score:
                neg_filename = all_doc_filenames[doc_index[doc_ids[j]]]
                if neg_filename not in image_index:
                    continue
                neg_idxs.append(image_index[neg_filename])
                if len(neg_idxs) >= NUM_NEGATIVES:
                    break

        # has_enough_negatives: require NUM_NEGATIVES valid negatives.
        if len(neg_idxs) < NUM_NEGATIVES:
            n_dropped_few_negs += 1
            continue

        rows["query"].append(all_queries[query_index[all_score_qids[i]]])
        rows["positive_image_idx"].append(image_index[pos_filename])
        rows["negative_image_idxs"].append(neg_idxs)
    ...
    def transform(batch):
        ...
        for n in range(NUM_NEGATIVES):
            out[f"negative_{n}"] = []
        for pos_idx, neg_idxs in zip(
            batch["positive_image_idx"], batch["negative_image_idxs"]
        ):
            pos_pil = decode_image.decode_example(images_ds[pos_idx]["image"])
            out["image"].append({"image": pos_pil, "text": DOCUMENT_PROMPT})
            for n, neg_idx in enumerate(neg_idxs):
    ...
    log.info(
        "Split %s ready (%d rows, %d dropped for <%d nv_threshold negatives).",
        split,
        len(ds),
        n_dropped_few_negs,
        NUM_NEGATIVES,
