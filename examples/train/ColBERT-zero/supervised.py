...
def load_train_datasets(**kwargs):
    ...
    cache_dir = "./nomic_data_hf/supervised_cache"
...
def main():
    ...
    run_name = f"{model_shortname}-supervised-lr{lr}-bs{batch_size}-temp{temperature}"
    output_dir = "output/{model_shortname}/{run_name}"

    # Initialize model
    model = models.ColBERT(
        model_name_or_path=model_name+'/final',
        document_length=512
    )
