...
[XTR](https://arxiv.org/abs/2304.01982) (conteXtual Token Retrieval) is a ColBERT variant that, at retrieval time, only uses the scores of the top-k tokens retrieved by the ANN index for each query token to compute the MaxSim score. If a given document isn't retrieved in the top-k for a given query token, its score for that query token is imputed as the minimum score of the tokens that *were* retrieved.
However, to condition the model to this retrieval behavior, XTR adopts a modified training objective that simulates the sparsity of query token scores by masking out any query-document token score for which the document wasn't retrieved in the top-k for that query token out of all in-batch documents.

PyLate exposes XTR's training objective as a **drop-in replacement for ColBERT's score function** — model architecture, tokenizer, and trainer are unchanged. You only swap the `score_metric` of the loss:

- `scores.XTRScores` → use with `losses.Contrastive` / `losses.CachedContrastive`
- `scores.XTRKDScores` → use with `losses.Distillation`
...
model = models.ColBERT(model_name_or_path="answerdotai/ModernBERT-base")

train_loss = losses.Contrastive(
    model=model,
    score_metric=scores.XTRScores(k=256),
    temperature=0.05,
)
```

The rest of the training script (dataset, `SentenceTransformerTrainer`, `ColBERTCollator`, evaluator) is identical to the contrastive example above. The same `score_metric` argument is also accepted by `losses.CachedContrastive` if you want to combine XTR with GradCache for larger effective batch sizes.
...
train_loss = losses.Distillation(
    model=model,
    score_metric=scores.XTRKDScores(k=256),
)
```

`XTRKDScores` runs the same global top-$k$ scoring as `XTRScores` but returns each query's own $N$-way document scores `(Q, N)` — matching the shape that `losses.Distillation` expects from a teacher's score matrix.

### Setting Hyperparameters for XTR

The `k` hyperparameter controls how many `(query_token, doc_token)` matches survive globally across all `Q*N` in-batch documents:

- **Too small:** most query tokens have no surviving match for the positive document, the score collapses toward the min-imputed value, and gradient signal degrades.
- **Too large:** XTR degenerates back into ColBERT's full MaxSim — losing the alignment with retrieval-time behavior.

The default of `k=256` is a reasonable starting point for typical batch sizes achievable on a single GPU.

While `k` is the only XTR-specific hyperparameter, we observe that in the contrastive setting, a temperature parameter lower than 1.0, e.g. 0.05, is beneficial for performance to mitigate the sparsity of the gradient signal caused by the many masked out token scores.
