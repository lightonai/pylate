    ...
    **Important Notes:**
        ...
        if self.use_autopilot:
            ...
            if self.verbose:
                ...
                if (
                    ...
                    or self.num_leaves_to_search is not None
            ...
            self.num_leaves = (
                ...
                else min(2_000, num_vectors)
            )
            self.num_leaves_to_search = (
                self.num_leaves_to_search
            ...
            self.training_sample_size = (
                ...
                else min(250000, num_vectors)
            )

            if self.verbose:
                logger.info(
                    f"[ScaNN] Building ScaNN searcher with {embeddings.shape[0]} vectors..."
                )
                logger.info(
                    f"[ScaNN]   Parameters: num_leaves={self.num_leaves}, num_leaves_to_search={self.num_leaves_to_search}, training_sample_size={self.training_sample_size}, num_neighbors={self.num_neighbors}"
                )
        ...
        if self.use_autopilot:
            searcher = (
                scann.scann_ops_pybind.builder(
                    embeddings, self.num_neighbors, "dot_product"
                )
                .autopilot()
                .build()
            )
    ...
    def _load_index(self) -> None:
        ...
        try:
            ...
            with open(metadata_path, "r") as f:
                ...
                self.embedding_size = metadata.get(
                ...
                self.num_leaves_to_search = metadata.get(
                    "num_leaves_to_search", self.num_leaves_to_search
                )
                self.training_sample_size = metadata.get(
                    "training_sample_size", self.training_sample_size
                )
                self.use_autopilot = metadata.get("use_autopilot", self.use_autopilot)
                self.store_embeddings = metadata.get(
            ...
            if self.store_embeddings and flattened_embeddings_path.exists():
                if self.verbose:
                    logger.info(
                        "[ScaNN] Loading flattened_embeddings from %s...",
                        flattened_embeddings_path,
                    )
                self.flattened_embeddings = np.load(flattened_embeddings_path)
                if self.verbose:
                    logger.info(
                        "[ScaNN] Loaded flattened_embeddings with shape %s",
                        self.flattened_embeddings.shape,
                    )
            else:
            ...
            if self.verbose:
                ...
                logger.info(
                    f"[ScaNN]   Documents: {len(self.doc_id_to_embedding_range)}"
                )
                logger.info(
    ...
    def save(self) -> None:
        ...
        if self.searcher is None:
            raise ValueError(
                "Cannot save index: no searcher has been built. Add documents first."
            )
    ...
    def add_documents(
        ...
        if np_dtype is None:
            raise ValueError(
                ...
                f"Got dtype={first_emb.dtype}."
            )

        if self.verbose:
            size_gb = (
                total_embeddings * embedding_dim * np.dtype(np_dtype).itemsize / 1e9
            )
            logger.info(
        ...
        for i, emb in iterator:
            if emb.shape[1] != embedding_dim:
                raise ValueError(
                    "All document embeddings must have the same embedding dimension. "
            ...
            if emb_np_dtype != np_dtype:
                raise ValueError(
                    "All document embeddings must have the same dtype. "
                    f"Expected {np_dtype}, got {emb_np_dtype}."
                )
            n = emb_np.shape[0]
    ...
    def remove_documents(self, documents_ids: list[str]) -> None:
        ...
        """
        raise NotImplementedError("Document removal is not supported for ScaNN index.")
    ...
    def __call__(
        ...
        # using np.split to avoid a per-token Python loop.
    ...
    def get_documents_embeddings(
        ...
        if self.flattened_embeddings is None:
            raise ValueError(
                "Index is empty, add documents before retrieving embeddings."
            )
