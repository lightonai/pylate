...
def process_page(inp):
    ...
    assert all(len(psg) in [len(words), nwords] for psg in passages), (
        list(map(len, passages)),
        len(words),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format", dest="format", required=True, choices=[Format1, Format2, Format3]
    )

    # Output Arguments.
    parser.add_argument(
        "--use-wordpiece", dest="use_wordpiece", default=False, action="store_true"
    )
