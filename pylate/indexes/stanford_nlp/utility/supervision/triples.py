...
def sample_negatives(negatives, num_sampled, biased=None):
    assert biased in [
        ...
        100,
        200,
    ], "NOTE: We bias 50% from the top-200 negatives, if there are twice or more."
    ...
    if biased and num_sampled < len(negatives):
        ...
        if len(oversampled) < len(undersampled):
            return random.sample(oversampled, num_sampled_top100) + random.sample(
                undersampled, num_sampled_rest
            )
    ...
    for pid, rank, *_, label in ranking:
        ...
        if label:
            take_this_positive = any(
                rank <= maxDepth and len(positives) < maxBest
                for maxBest, maxDepth in args_positives
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--depth", dest="depth", required=True, type=int
    )  # for negatives

    parser.add_argument(
        "--permissive", dest="permissive", default=False, action="store_true"
    )
    ...
    args.positives = [
        list(map(int, configuration.split(","))) for configuration in args.positives
    ]

    assert all(len(x) == 2 for x in args.positives)
    assert all(
        maxBest <= maxDepth for maxBest, maxDepth in args.positives
    ), args.positives
