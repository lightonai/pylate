...
class HyperEncoderModule(nn.Module):
    ...
    def __init__(self, converter_kwargs: dict, base_encoder_output_dim: int):
        ...
        self.vector_dimensions = converter_kwargs.get("vector_dimensions", [128, 64, 1])
        ...
        nn.init.normal_(self.param_generator.weight, std=0.1)
        nn.init.zeros_(self.param_generator.bias)
        
        # Template q-net for structure (we'll override its parameters)
        self.template_qnet = TokenWiseQNet(converter_kwargs)
    def forward(self, features):
        # Pass through - not used in main pipeline
        return features
...
class HyperColBERT(ColBERT):
    ...
    def __init__(
        ...
        **kwargs
        ...
        converter_kwargs = converter_kwargs or {"vector_dimensions": [128, 64, 1]}
        self.vector_dimensions = [128, 64, 1] #converter_kwargs.get("vector_dimensions", [128, 64, 1])
